...
class AddressTheFoodTests(unittest.TestCase):
    ...
    def test_picks_food_refills_and_plays_it_one_energy_cheaper(self) -> None:
        game = empty_game()
    ...
    def test_keeps_food_in_hand_when_discounted_cost_is_unaffordable(self) -> None:
        game = empty_game()
        game.ais[0] = FirstSuitcaseTargetAI(game.rng)
        player = game.players[0]
        player.energy = 2
        ...
        self.assertEqual(player.hand, [food])
        self.assertEqual(game.stats.card_plays[food.title], 0)
        self.assertEqual(player.picked_cards[food.title], 1)
        self.assertIs(game.suitcase[1], refill)
