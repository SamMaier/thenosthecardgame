...
class MorningWalkTests(unittest.TestCase):
    def test_printed_values_and_first_card_restriction(self) -> None:
        game = empty_game()
        player = game.players[0]
        player.energy = 7
        player.hand.extend([make_card("biography"), make_card("morning-walk")])
        ...
        card = player.hand[0]
        self.assertEqual(card.title, "Morning Walk")
    ...
    def test_first_play_cost_and_tomorrow_energy(self) -> None:
        ...
        player = game.players[0]
        player.energy = 7
        player.hand.append(make_card("morning-walk"))
